...
class NERClassifier:
    ...
    def create_model(self, embeddings, emb_features, feature_size, output_size):    # pylint: disable=too-many-locals
        ...
        rnn = Bidirectional(
            GRU(ModelParameters.output_dim_rnn, activation=ModelParameters.activation_rnn,
                return_sequences=True))(mask_in)
        dropout = Dropout(ModelParameters.dropout)(rnn)
        time_distributed_dense = TimeDistributed(
            Dense(ModelParameters.dense_size, activation=ModelParameters.activation_dense), )(dropout)
        ...
        model.compile(optimizer=ModelParameters.optimizer, loss=ModelParameters.loss_function, metrics=['accuracy'])
        model.summary()
