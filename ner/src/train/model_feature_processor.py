...
class TrainFeaturesProcessor:
    ...
    def get_additional_features(self, sentences: list[Sentence], dep2idx: dict[str, int]) -> ndarray:
        ...
        dependency_labels = np.array([
            self.one_hot_encode(sentence.input.get_padded_dependency_labels_idx(padding), dep2idx)
    ...
    def get_input(self, sentences: list[Sentence], vocab: Embedding) -> ndarray:
        padding = ModelParameters.padding
        return np.array([
            sentence.input.get_padded_words_idx(padding, vocab.vocabulary[Constants.pad_word]) for sentence in sentences
        ])
    ...
    def one_hot_encode(self, labels: list[int], label_to_idx: dict[str, int]) -> ndarray:
        one_hot_categories = OneHotEncoder(sparse_output=False)
        one_hot_categories.fit([[idx] for idx in label_to_idx.values()])
        return one_hot_categories.transform([[label] for label in labels])
