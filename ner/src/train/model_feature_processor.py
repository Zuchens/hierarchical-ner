...
class TrainFeaturesProcessor:
    ...
    def get_additional_features(self, sentences: list[Sentence], dep2idx: dict[str, int]) -> ndarray:
        ...
        numerical_features = np.array([sentence.get_padded_features(padding) for sentence in sentences])
        dependency_idx = np.reshape(np.array([sentence.get_padded_dependencies(padding) for sentence in sentences]),
                                    (numerical_features.shape[0], numerical_features.shape[1], 1))
        dependency_labels = np.array([self.one_hot_encode(sentence.get_padded_dependency_labels_idx(padding), dep2idx)
                                      for sentence in sentences])
        features = np.concatenate((numerical_features, dependency_idx, dependency_labels), axis=2)
        return features

    def prepare_data(self, data_train: list[Sentence],
                     preprocessed_data: PreprocessedData,
                     vocab: Embedding) -> ModelData:
        numerical_features = self.get_additional_features(data_train,
                                                          preprocessed_data.dependency_label_to_idx)
    ...
    def get_input(self, sentences: list[Sentence], vocab: Embedding) -> ndarray:
        padding = ModelParameters.padding
        return np.array([
            sentence.get_padded_words_idx(padding, vocab.vocabulary[Constants.pad_word])
            for sentence in sentences
        ])
    ...
    def get_targets(self, label_to_idx: dict[str, int], sentences: list[Sentence]) -> ndarray:
        padding = ModelParameters.padding
        target = [
            self.one_hot_encode(sentence.get_padded_target(padding), label_to_idx)
            for sentence in sentences
        ]
